...
def _prepare_files_labels(
    ...
    filenames = [
        ...
        if f.suffix.lower() in extensions and not f.is_dir()
    ]
    if len(filenames) == 0:
...
def make_dataset(
    ...
    missing_files: set = None,
):
    """Make Folder Dataset.

    Args:
        normal_dir (Union[str, Path]): Path to the directory containing normal images.
        abnormal_dir (Optional[Union[str, Path]]): Path to the directory containing abnormal images.
    ...
    if mask_dir is not None:
        data_dir = _check_and_convert_path(normal_dir).parent
        mask_dir = data_dir / mask_dir
        samples["mask_path"] = ""
        for index, row in samples.iterrows():
            if row.label_index != 1:
                continue

            mask_path = mask_dir / row.image_path.name

            if not mask_path.exists():
                if str(mask_path) not in missing_files:
                    click.secho(
                        f"WARNING: no mask image detected for bad sample"
                        f" image '{row.image_path.name}' located at '{mask_path}'. "
                        "Ignoring this image.",
                        bold=True,
                        fg="yellow",
                    )
                missing_files.add(str(mask_path))
                samples.drop(index=index, inplace=True)
                continue

            samples.loc[index, "mask_path"] = str(mask_path)
...
class FolderDataset(Dataset):
    ...
    def __init__(
        ...
        abnormal_dir: Optional[Union[Path, str]],
        category: str,
        split: str,
...
class FolderDataModule(LightningDataModule):
    ...
    def __init__(
        ...
        abnormal_dir: Optional[str] = None,
        ...
        Args:
            ...
            normal_dir (str, optional): Name of the directory containing normal images.
                Defaults to "normal".
            abnormal_dir (Optional[str], optional): Name of the directory containing abnormal images.
                Defaults to None.
        ...
        Examples:
            ...
            >>> from hamacho.core.data import FolderDataModule
        ...
        self.category = category
        self.normal_dir = self.root / self.category / normal_dir
        self.abnormal_dir = (
            abnormal_dir
            if abnormal_dir is None
            else self.root / self.category / abnormal_dir
        )
