    ...
    if "order_of_parts" in optimisation_parts:
        ...
        for part in enabled_options:
            assert part in order_of_parts, (
                ...
                "but it is absent in order_of_parts={order_of_parts}"
            )

        optimisation_parts_to_choose = [
            part for part in order_of_parts if part in enabled_options
        ]

    assert (
        "base" in optimisation_parts
    ), 'Error: the optimisation config does not contain the "base" part'
    nncf_config_part = optimisation_parts["base"]

    for part in optimisation_parts_to_choose:
        assert (
            part in optimisation_parts
        ), f'Error: the optimisation config does not contain the part "{part}"'
        optimisation_part_dict = optimisation_parts[part]
        try:
            nncf_config_part = merge_dicts_and_lists_b_into_a(
                nncf_config_part, optimisation_part_dict
            )
...
def _merge_dicts_and_lists_b_into_a(a, b, cur_key=None):
    ...
    * overwrites when merging scalar into scalar
    ...
    assert isinstance(
        a, (dict, list)
    ), f"Can merge only dicts and lists, whereas type(a)={type(a)}"
