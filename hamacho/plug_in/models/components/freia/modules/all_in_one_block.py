...
from hamacho.plug_in.models.components.freia.modules.base import InvertibleModule
...
class AllInOneBlock(InvertibleModule):
    ...
    def __init__(
        ...
        self.global_scale = nn.Parameter(
            torch.ones(1, self.in_channels, *([1] * self.input_rank))
            * float(global_scale)
        )
    ...
    def _construct_householder_permutation(self):
        ...
        for vk in self.vk_householder:
            w = torch.mm(
                w,
                torch.eye(self.in_channels).to(w.device)
